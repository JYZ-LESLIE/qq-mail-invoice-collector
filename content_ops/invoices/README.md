...
- 服务端过滤：邮件进入解析链路前，会先用 IMAP UID SEARCH 按 `START_DATE`、主题关键字和常见平台发件域名缩小范围；主题关键字覆盖 `发票`、`电子发票`、`开票`、`Invoice`、`账单`、`行程单`、`滴滴`、`小桔`、`曹操出行`、`高德打车`、`T3出行`、`首汽约车`、`出行发票`、`行程报销凭证`、`飞书`、`Lark`、`火山引擎` 等。
