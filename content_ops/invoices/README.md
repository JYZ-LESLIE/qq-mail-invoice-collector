...
- 服务端过滤：邮件进入解析链路前，会先用 IMAP UID SEARCH 按 `START_DATE`、标题关键字、正文关键字和常见平台发件域名缩小范围；标题和正文核心召回词覆盖 `发票`、`电子发票`、`发票下载`、`下载发票`、`开票`、`开票成功`、`开具成功`、`Invoice`、`Tax Invoice`，标题平台词继续覆盖 `账单`、`行程单`、`飞书`、`Lark`、`火山引擎` 等。
...
- 断点补漏：之前被初筛跳过的 `skipped_subject_filter` 不再视为最终完成，规则升级后普通小范围扫描也能重新检查这类候选；大范围补漏仍建议带 `--reprocess`。
